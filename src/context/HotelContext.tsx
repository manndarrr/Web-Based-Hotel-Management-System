...
import { getDb, insertBooking, getAllBookings, updateBookingStatus as dbUpdateBookingStatus, insertFoodOrder, saveDb, type DbBooking } from '@/services/database';
...
function generateCostQueries(): CostQuery[] {
  return [
    { id: 1, bookingId: 1, guestName: 'Arjun Mehta', description: 'Extra bed arrangement for child', amount: 1500, status: 'Pending', createdAt: new Date().toISOString() },
    { id: 2, bookingId: 2, guestName: 'Priya Sharma', description: 'Airport pickup service', amount: 2500, status: 'Pending', createdAt: new Date().toISOString() },
    { id: 3, bookingId: 3, guestName: 'David Wilson', description: 'Late checkout request (2 PM)', amount: 1000, status: 'Accepted', createdAt: new Date(Date.now() - 86400000).toISOString() },
    { id: 4, bookingId: 4, guestName: 'Riya Patel', description: 'Spa & wellness package', amount: 4500, status: 'Rejected', createdAt: new Date(Date.now() - 172800000).toISOString() },
    { id: 5, bookingId: 5, guestName: 'James Brown', description: 'Room upgrade to Royal Suite', amount: 8000, status: 'Pending', createdAt: new Date().toISOString() },
  ];
}
...
interface HotelContextType extends HotelState {
  ...
  staffLogout: () => void;
  updateCostQuery: (id: number, status: 'Accepted' | 'Rejected') => void;
...
export function HotelProvider({ children }: { children: React.ReactNode }) {
  ...
  // Initialize database and load bookings
  useEffect(() => {
    const init = async () => {
      try {
        await getDb();
  ...
  const updateBookingStatusFn = useCallback(async (id: number, status: 'Booked' | 'Active' | 'Completed' | 'Rejected') => {
    ...
    if (booking) {
      ...
      if (status === 'Active') {
        ...
        setRooms(prev => prev.map(r => r.id === roomId ? { ...r, status: 'Completed' as const } : r));
      }
    }
  ...
  const staffLogin = useCallback((password: string) => {
    if (password === 'admin123') { setStaffLoggedIn(true); return true; }
    return false;
  }, []);

  const updateCostQuery = useCallback((id: number, status: 'Accepted' | 'Rejected') => {
    setCostQueries(prev => prev.map(q => q.id === id ? { ...q, status } : q));
  }, []);
  ...
  return (
    <HotelContext.Provider value={{
      rooms, bookings, foodOrders, costQueries, inventory, activeBookingId, staffLoggedIn, dbBookings, dbLoading,
      bookRoom, addFoodOrders, checkout: checkoutFn, setActiveBooking: setActiveBookingId,
      staffLogin, staffLogout: () => setStaffLoggedIn(false), updateCostQuery, updateInventory, getMenu, getRoomCategories,
      updateBookingStatus: updateBookingStatusFn, refreshDbBookings,
    }}>
      {children}
